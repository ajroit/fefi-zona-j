#!/usr/bin/env python3
"""
Fetcher de datos Futsal - Torneo de Duelos
Consume la API pública de Weball (api.weball.me) y genera data/futsal-duelos-data.json
"""
...
TOURNAMENT_ID = 843        # Torneo de Duelos
...
OUTPUT_FILE = os.path.join(BASE_DIR, "data", "futsal-duelos-data.json")
...
# ── 1. Obtener fases ──────────────────────────────────────
def obtener_fases():
    """Devuelve lista de fases (ej: [{id: 1295, name: 'FECHA DE DUELOS', ...}])."""
...
def procesar_fixture(visualizer_data, categorias, phase_id):
    ...
    for child in visualizer_data.get("children", []):
        ...
        try:
            ...
            pass
        if num == 0:
            num = 1 # Fallback para Fecha de Duelos
...
# ── Main ──────────────────────────────────────────────────
def main():
    print("🏟️  Fetcher Futsal - Torneo de Duelos")
    print("=" * 50)
